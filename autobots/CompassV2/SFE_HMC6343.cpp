...
void SFE_HMC6343::changeAddr(uint8_t add)
{
	_addr = add;
}
void SFE_HMC6343::findAddr()
{
	uint8_t data;
	for(uint8_t i = START_ADDR; i<=END_ADDR; i++)
	{
		...
		if(data == (i << 1))
		{
			Serial.println("input address: ");
			Serial.println(i,HEX);
			Serial.println("data not shitfted: ");
...
void SFE_HMC6343::getData(byte* buff)
{
	

  sendCommand(POST_HEADING); // Send specified I2C command to HMC6343
  delay(1); // Delay response time
